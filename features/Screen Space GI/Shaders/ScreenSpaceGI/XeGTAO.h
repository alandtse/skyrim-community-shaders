...
#define XE_GTAO_NUMTHREADS_X 32     // these can be changed
#define XE_GTAO_NUMTHREADS_Y 32     // these can be changed
	...
	struct ReferenceRTAOConstants
	{
		...
		int Padding2;
#ifdef __cplusplus
		ReferenceRTAOConstants()
		{
			TotalRaysLength = 1.0f;
			Albedo = 0.0f;
			MaxBounces = 1;
			AccumulatedFrames = 0;
			AccumulateFrameMax = 0;
		}
#endif
	};
...
#	ifdef IMGUI_API
	inline bool GTAOImGuiSettings(XeGTAO::GTAOSettings& settings)
	{
		...
		if (ImGui::IsItemHovered())
			ImGui::SetTooltip("Higher quality settings use more samples per pixel but are slower");
		...
		if (ImGui::IsItemHovered())
			ImGui::SetTooltip("The amount of edge-aware spatial denoise applied");
		...
		if (ImGui::CollapsingHeader("Auto-tuned settings (heuristics)")) {
			...
			if (ImGui::IsItemHovered())
				ImGui::SetTooltip("Multiplies the 'Effect Radius' - used by the auto-tune to best match raytraced ground truth\nExpected range: [0.3, 3.0], defaults to %.3f", XE_GTAO_DEFAULT_RADIUS_MULTIPLIER);
