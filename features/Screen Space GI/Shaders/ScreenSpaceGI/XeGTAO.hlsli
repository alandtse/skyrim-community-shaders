...
void GatherFour(Texture2D<float4> tex, int2 coords, out AOTermType outTerm[4])
{
	coords -= 1;
	outTerm[0] = tex[coords + int2(0, 1)];
	outTerm[1] = tex[coords + int2(1, 1)];
	outTerm[2] = tex[coords + int2(1, 0)];
	outTerm[3] = tex[coords + int2(0, 0)];
}

void XeGTAO_Denoise(const uint2 pixCoordBase, const GTAOConstants consts, Texture2D<float4> sourceAOTerm, Texture2D<lpfloat> sourceEdges, SamplerState texSampler, RWTexture2D<float4> outputTexture, const uniform bool finalApply)
{
	...
	for (int side = 0; side < 2; side++) {
		...
		lpfloat edginess = (saturate(4.0 - leak_threshold - dot(edgesC_LRTB[side], 1.f)) / (4 - leak_threshold)) * leak_strength;
